...
sudo docker run -d --rm --net=ros --env="DISPLAY_WIDTH=3000" --env="DISPLAY_HEIGHT=1800" --env="RUN_XTERM=no" --name=novnc -p=8080:8080 theasp/novnc:latest  && \
sleep 2 &&\
sudo docker run -d --name turtles_example --rm --net=ros --env="DISPLAY=novnc:0.0" --env="ROS_MASTER_URI=http://localhost:11311" -p11311:11311 -p9090:9090 maiquelb/embedded-mas-ros2:0.5 /bin/bash -c '((source /opt/ros/humble/setup.bash && ros2 run  _node) & (source /opt/ros/humble/setup.bash && cd ~/ &&\
...
| /turtleX/alarm
| std_msgs/String
| Status of an alarm that monitors the environmental safety. Possible values of this topic are `safe` and `critical`
|===

=== Task 3.1: Configuring perceptions

* Exercise 3.1.1. Open the JaCaMo application at link:https://github.com/embedded-mas/jason-ros-course-wesaac2024/tree/main/hands-on/part3/ex1[`part3/ex1`]. The file link:https://github.com/embedded-mas/jason-ros-course-wesaac2024/blob/main/hands-on/part3/ex1/src/agt/ros_agent.asl[`src/agt/ros_agent.asl`] is the source code of an agent must just print its current position. The plan that implements this behaviour is right below the comment `//Exercise 3.1.1`. Run the application and check whether the robot behaves as expected. If yes, 
