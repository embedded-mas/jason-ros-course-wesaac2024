...
ros2 run embedded_mas_examples service) & (/bin/bash -c "source /opt/ros/humble/setup.bash && ros2 launch rosbridge_server rosbridge_websocket_launch.xml")) && echo "READY")  &\ 
wait '
----
...
In this tutorial, we will develop a BDI agent that moves in a square environment. The cognitive portion of the agent is programmed with Jason. The body of the agent is a turtlebot.
