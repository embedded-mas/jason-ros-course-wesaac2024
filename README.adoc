...
* Exercise 3.2.1: make the agent to use the action `move_to` move to the coordinates (2,2). This action must be realized through the ROS service `/turtleX/teleport_absolute`. This requires changes in the code of the agent (link:https://github.com/embedded-mas/jason-ros-course-wesaac2024/blob/main/hands-on/part3/ex2/src/agt/ros_agent.asl[.asl file]) and in the connection between the action `move_to` and the ROS service that actually realizes it, specified in the link:https://github.com/embedded-mas/jason-ros-course-wesaac2024/blob/main/hands-on/part3/ex2/src/agt/robot1.yaml[.yaml file]. The comment "TODO: Exercice 3.2.1:" indicates the place where the changes must be implemented in both the files.
